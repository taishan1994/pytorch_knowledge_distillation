...
from config.config_with_attack import BertForClassificationConfig, BilstmForClassificationConfig
...
from utils.attack_utils import FGM, PGD
            ...
            for train_step, train_data in enumerate(train_loader):
                ...
                if fgm is not None:
                    ...
                    if model_name == "bert":
                        ...
                        fgm.attack(emb_name="embedding.weight")
                        if self.config.use_apex:
                            with autocast():
                                train_outputs = self.model(input_ids, sentence_lengths)
                                loss = self.criterion(train_outputs, label_ids)
                        ...
                        if self.config.use_apex:
                            ...
                            loss.backward()
                        fgm.restore(emb_name="embedding.weight")  # 恢复Embedding的参数
                elif pgd is not None:
                    # 1、保存正常的grad
                    pgd.backup_grad()
                    if model_name == "bert":
                        for t in range(pgd_k):
                            # 2、在embedding上添加对抗扰动, first attack时备份param.data
                        ...
                        for t in range(pgd_k):
                            # 2、在embedding上添加对抗扰动, first attack时备份param.data
                            pgd.attack(emb_name="embedding.weight", is_first_attack=(t == 0))
                            if t != pgd_k - 1:
                                self.optimizer.zero_grad()
                            else:
                                pgd.restore_grad()
                            ...
                            if self.config.use_apex:
                                ...
                                loss.backward()
                        # 5、恢复embedding参数
                        pgd.restore(emb_name="embedding.weight")
